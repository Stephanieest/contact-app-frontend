...
function FavoriteForm() {
  ...
  const loadFavorites = async () => {
    try {
      const response = await fetch('https://ringzingg.onrender.com/favorites');
      if (!response.ok) {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      ...
      const endpoint = editMode ? `https://ringzingg.onrender.com/favorites${currentFavoriteId}` : 'https://ringzingg.onrender.com/favorites';
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`https://ringzingg.onrender.com/favorites${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
